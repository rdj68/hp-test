...
window.onload = () => {
  ...
  $("#submit").on("click", function (e) {
    ...
    const weightedMean = Number(
      (weightedScores.reduce((a, b) => a + b) / summationWeight).toFixed(2)
    );
...
function createQuestionElement(question, questionNumber) {
  ...
  const header = $(
    `<h2 class="subtitle is-size-4">Question #${questionNumber}</h2>`
  );
  qElement.append(header);
  qElement.append(
    $('<h3 class="is-size-5 has-text-weight-bold mb-3">').append(
      question.question
    )
  );
...
function displayQuestion() {
  ...
  quizView.fadeOut(function () {
    ...
    quizView
      .append(createQuestionElement(question, currentQuestionIndex + 1))
      .fadeIn();
  ...
  get(averageRef).then((snap) => {
    ...
    if (state["totalUsers"]) {
      update(averageRef, {
        ...
        averageScore:
          (state["averageScore"] * state["totalUsers"] + score * 100) /
          (state["totalUsers"] + 1),
      });
    } else {
      update(averageRef, {
  ...
  get(averageRef).then((snap) => {
    ...
    if (school["totalUsers"]) {
      update(averageRef, {
        ...
        averageScore:
          (school["averageScore"] * school["totalUsers"] + score * 100) /
          (school["totalUsers"] + 1),
      });
    } else {
      update(averageRef, {
